...
**`control-api` is a separate Node.js service maintained outside this repo** — it is not implemented here. This repo only has the client side: `proto/control_api.proto` (the shared contract — protobuf is language-agnostic, so this same file is what the Node.js server should codegen from too) and `src/control_api.py` (Python gRPC client: `fetch_config()`, `ConfigWatcher`). `src/main.py` is the pipeline entrypoint: on startup it calls `control_api.fetch_config()` to get the full config (`input`/`pipeline`/`output`) from wherever control-api is reachable (`CONTROL_API_HOST`/`CONTROL_API_PORT` env vars), builds the pipeline, serves the MJPEG viewer (`webview.py`), and subscribes to config changes via `control_api.ConfigWatcher` (a `WatchConfig` streaming RPC). There is no `config.yaml` and no local default config — `src/config.py` holds only `compute_derived()` (pure fn, fills `num_sources`/`batched_push_timeout`/`pipeline.tiler`) and the `resolve()` path helper; control-api is the sole source of the actual config values, no local fallback exists (the pipeline blocks on `GetConfig` at startup and errors out if control-api is unreachable — there is no `--no-control-api` bypass). `src/state.py`'s `apply_config()` replaces the whole live config wholesale with whatever control-api sent (not a partial merge). Don't assume any other file exists until verified — check first.
...
Config schema follows `input`/`pipeline`/`output` sections (no YAML — see `src/config.py`/`src/state.py` above). **Update, not in the original guide's scope**: a gRPC control-api (Node.js, separate repo — not here, see `proto/control_api.proto` + Project overview above) now exists, added deliberately as a learning exercise for gRPC, not because the pipeline needed it. It owns the entire config (`input` + `pipeline` — including per-model `inference` map, `tracker`, `osd`, `zone` — + `output`) as the single source of truth; only derived-only values (`num_sources`, `batched_push_timeout`, `pipeline.tiler`) stay local, computed by `compute_derived()` after every config apply:
